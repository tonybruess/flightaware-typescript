...
```sh
npm install git+ssh://git@github.com:stainless-sdks/flightaware-typescript.git
```

> [!NOTE]
> Once this package is [published to npm](https://app.stainless.com/docs/guides/publish), this will become: `npm install flightaware`
...
await client.flights.search.perform({
  maxRetries: 5,
});
```
...
const response = await client.flights.search.perform().asResponse();
console.log(response.headers.get('X-My-Header'));
console.log(response.statusText); // access the underlying Response object

const { data: response, response: raw } = await client.flights.search.perform().withResponse();
...
1. Via the `FLIGHTAWARE_LOG` environment variable
...
const client = new Flightaware({
  logger: logger.child({ name: 'Flightaware' }),
...
const client = new Flightaware({ fetch });
```
...
```ts
import Flightaware from 'npm:flightaware';
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks/flightaware-typescript/issues) with questions, bugs, or suggestions.
