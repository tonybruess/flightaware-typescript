...
import { APIPromise } from 'flightaware/core/api-promise';

import util from 'node:util';
import Flightaware from 'flightaware';
import { APIUserAbortError } from 'flightaware';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['FLIGHTAWARE_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: Flightaware) => {
    ...
    test('debug logs when log level is debug', async () => {
      ...
      const client = new Flightaware({ logger: logger, logLevel: 'debug', apiKey: 'My API Key' });
    ...
    test('debug logs are skipped when log level is info', async () => {
      ...
      const client = new Flightaware({ logger: logger, logLevel: 'info', apiKey: 'My API Key' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['FLIGHTAWARE_LOG'] = 'debug';
      const client = new Flightaware({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['FLIGHTAWARE_LOG'] = 'not a log level';
      const client = new Flightaware({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'FLIGHTAWARE_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
    ...
    test('client log level overrides env var', async () => {
      ...
      process.env['FLIGHTAWARE_LOG'] = 'debug';
      const client = new Flightaware({ logger: logger, logLevel: 'off', apiKey: 'My API Key' });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new Flightaware({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['FLIGHTAWARE_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new Flightaware({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['FLIGHTAWARE_BASE_URL'] = 'https://example.com/from_env';
      const client = new Flightaware({ apiKey: 'My API Key' });
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Flightaware({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Flightaware({ apiKey: 'My API Key' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['FLIGHTAWARE_API_KEY'] = 'My API Key';
    const client = new Flightaware();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['FLIGHTAWARE_API_KEY'] = 'another My API Key';
    const client = new Flightaware({ apiKey: 'My API Key' });
    expect(client.apiKey).toBe('My API Key');
  });
});
...
describe('retries', () => {
  test('retry on timeout', async () => {
    ...
    const client = new Flightaware({ apiKey: 'My API Key', timeout: 10, fetch: testFetch });
  ...
  test('retry count header', async () => {
    ...
    const client = new Flightaware({ apiKey: 'My API Key', fetch: testFetch, maxRetries: 4 });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Flightaware({ apiKey: 'My API Key', fetch: testFetch });
