...
import { Account, AccountGetUsageParams, AccountGetUsageResponse } from './resources/account';
import {
  Aircraft,
  AircraftCheckBlockedResponse,
  AircraftGetOwnerResponse,
  AircraftGetTypeInfoResponse,
} from './resources/aircraft';
import {
  DisruptionCountRetrieveEntityParams,
  DisruptionCountRetrieveEntityResponse,
  DisruptionCountRetrieveGlobalParams,
  DisruptionCountRetrieveGlobalResponse,
  DisruptionCounts,
} from './resources/disruption-counts';
import { ScheduleRetrieveParams, ScheduleRetrieveResponse, Schedules } from './resources/schedules';
import {
  AirportGetCanonicalCodeParams,
  AirportGetCanonicalCodeResponse,
  AirportGetRoutesParams,
  AirportGetRoutesResponse,
  AirportListParams,
  AirportListResponse,
  AirportRetrieveResponse,
  Airports,
} from './resources/airports/airports';
import {
  AlertCreateParams,
  AlertListParams,
  AlertListResponse,
  ...
  AlertUpdateParams,
  Alerts,
} from './resources/alerts/alerts';
import {
  FlightGetCanonicalIdentParams,
  FlightGetCanonicalIdentResponse,
  FlightGetCurrentPositionResponse,
  FlightGetFiledRouteResponse,
  FlightGetTrackMapParams,
  FlightGetTrackMapResponse,
  FlightGetTrackParams,
  FlightGetTrackResponse,
  FlightRetrieveParams,
  FlightRetrieveResponse,
  FlightSubmitIntentParams,
  Flights,
} from './resources/flights/flights';
import { Foresight } from './resources/foresight/foresight';
import { History } from './resources/history/history';
import {
  OperatorGetCanonicalParams,
  OperatorGetCanonicalResponse,
  OperatorListParams,
  OperatorListResponse,
  OperatorRetrieveResponse,
  Operators,
} from './resources/operators/operators';
...
export interface ClientOptions {
  ...
  /**
   * Defaults to process.env['FLIGHTAWARE_ENV'].
   */
   ...
   *
   * Defaults to process.env['FLIGHTAWARE_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['FLIGHTAWARE_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Flightaware API.
 */
export class Flightaware {
   ...
   */
  constructor({
    baseURL = readEnv('FLIGHTAWARE_BASE_URL'),
    apiKey = readEnv('FLIGHTAWARE_API_KEY'),
    env = readEnv('FLIGHTAWARE_ENV') ?? 'aeroapi',
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.FlightawareError(
        "The FLIGHTAWARE_API_KEY environment variable is missing or empty; either provide it, or instantiate the Flightaware client with an apiKey option, like new Flightaware({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? Flightaware.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('FLIGHTAWARE_LOG'), "process.env['FLIGHTAWARE_LOG']", this) ??
      defaultLogLevel;
  ...
  #baseURLOverridden(): boolean {
    return this.baseURL !== 'https://aeroapi.flightaware.com/aeroapi';
  }
  ...
  static Flightaware = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static FlightawareError = Errors.FlightawareError;
  ...
  flights: API.Flights = new API.Flights(this);
  foresight: API.Foresight = new API.Foresight(this);
  airports: API.Airports = new API.Airports(this);
  operators: API.Operators = new API.Operators(this);
  alerts: API.Alerts = new API.Alerts(this);
  history: API.History = new API.History(this);
...
Flightaware.Operators = Operators;
Flightaware.Alerts = Alerts;
Flightaware.History = History;
Flightaware.Aircraft = Aircraft;
...
export declare namespace Flightaware {
  ...
  export {
    DisruptionCounts as DisruptionCounts,
