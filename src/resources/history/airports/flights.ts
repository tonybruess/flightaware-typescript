...
export class Flights extends APIResource {
  /**
   * Returns all flights that were planned to arrive at a given airport between the
   * requested start and end date, including canceled flights. The span between
   * `start` and `end` date parameters can be up to 1 day (24 hours). No more than 40
   * pages may be requested at once. Data is available between 2011-01-01 00:00:00
   * UTC and up to 15 days prior to current datetime. The field
   * `inbound_fa_flight_id` will not be populated by this resource.
   *
   * @example
   * ```ts
   * const response =
   *   await client.history.airports.flights.arrivals('id', {
   *     end: {},
   *     start: {},
   *   });
   * ```
  ...
  /**
   * Returns all flights that were planned to depart from a given airport between the
   * requested start and end date, including canceled flights. The span between
   * `start` and `end` date parameters can be up to 1 day (24 hours). No more than 40
   * pages may be requested at once. Data is available between 2011-01-01 00:00:00
   * UTC and up to 15 days prior to current datetime. The field
   ...
   * ```ts
   * const response =
   *   await client.history.airports.flights.departures('id', {
   *     end: {},
   *     start: {},
   *   });
   * ```
...
export interface FlightArrivalsResponse {
  arrivals: Array<FlightArrivalsResponse.Arrival>;

  /**
   * Object containing links to related resources.
   */
  links: FlightArrivalsResponse.Links | null;
...
export namespace FlightArrivalsResponse {
...
export interface FlightDeparturesResponse {
  departures: Array<FlightDeparturesResponse.Departure>;

  /**
   * Object containing links to related resources.
   */
  links: FlightDeparturesResponse.Links | null;
...
export namespace FlightDeparturesResponse {
...
export interface FlightArrivalsParams {
  /**
   * The ending date range for flight results, comparing against flight's
   * `scheduled_on` field (which represents the flight’s planned ON the ground time),
   * The format is ISO8601 date or datetime, and the bound is exclusive. Specified
   * end date must occur after 2011-01-01 00:00:00 UTC and the most recent it can be
   * is 15 days ago from today. If using date instead of datetime, the time will
   * default to 00:00:00Z.
   */
  end: unknown;

  /**
   * The starting date range for flight results, comparing against flight's
   * `scheduled_on` field (which represents the flight’s planned ON the ground time).
   * The format is ISO8601 date or datetime, and the bound is inclusive. Specified
   * start date must occur on or after 2011-01-01 00:00:00 UTC and the most recent it
   * can be is 15 days ago from today. If using date instead of datetime, the time
   * will default to 00:00:00Z.
   */
  start: unknown;
...
export interface FlightDeparturesParams {
  /**
   * The ending date range for flight results, comparing against flight's
   * `scheduled_off` field (which represents the flight’s planned OFF the ground
   * time) The format is ISO8601 date or datetime, and the bound is exclusive.
   * Specified end date must occur after 2011-01-01 00:00:00 UTC and the most recent
   * it can be is 15 days ago from today. If using date instead of datetime, the time
   * will default to 00:00:00Z.
   */
  end: unknown;
   ...
   * The starting date range for flight results, comparing against flight's
   * `scheduled_off` field (which represents the flight’s planned OFF the ground
   * time). The format is ISO8601 date or datetime, and the bound is inclusive.
   * Specified start date must occur on or after 2011-01-01 00:00:00 UTC and the most
   * recent it can be is 15 days ago from today. If using date instead of datetime,
